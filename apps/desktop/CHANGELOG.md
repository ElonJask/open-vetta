...
- 手动拖拽侧边栏宽度不再滞后：拖拽途中实时宽度只写面板元素，不再每帧重渲染整条侧边栏、根布局与当前页面，主内容区也不再逐帧重排，松手才落定一次。长会话页快拖实测从 p50 57-71ms、38 个长任务共 3000ms 降到 p50 14.3ms、2 个长任务共 180ms，宽度跟上每一步指针移动。代价是拖拽途中内容区暂不跟随：变宽时面板盖在内容上，变窄时与内容之间短暂露出背景，松手归位。
