...
本目录记录 desktop 主题系统的设计与实施约定。这里的主题不是单纯的颜色 token，而是面向应用 UI 的装饰、复用和替换机制。

当前主题系统分三条线演进：

- 颜色主题：已有 `apps/desktop/src/renderer/shared/theme/`，负责 CSS token。
...
- [包边界与动态加载](./package-boundaries.md)：`theme-sdk`、`theme-ui`、desktop 和远程主题包的职责。
