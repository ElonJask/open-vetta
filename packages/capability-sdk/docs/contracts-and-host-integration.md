...
它不拥有 Plugin、Theme、Action 等上层系统的权限映射或 Adapter。上层系统依赖这些合同完成集成，
SDK 不反向认识调用者。

同样地，上层 facade 的身份和资源术语必须在系统集成层被消化。例如 Plugin 集成把当前 `pluginId`
...
```text
Plugin / Theme system facade (upper layer)
              |
              v
system integration owned by the upper layer
              |
              v
...
- Plugin/Theme/Action 的权限映射、身份绑定、Session 生命周期和 facade Adapter。
...
Theme Page 与 Plugin Workspace View 同时包含“宿主管理地址、扩展方提供页面”的结构，但实现必须保持
Desktop 基础设施、Capability 暴露层和来源系统 Adapter 三层分离。

进入 Domain Capability 的只是宿主无关、可序列化的导航意图：
...
DOMAIN_NAVIGATION_CAPABILITIES.OPEN_HOSTED_ROUTE;
```

`src/domain/navigation.ts` 是该合同的事实源。公共合同不枚举 Plugin/Theme，也不知道具体 URL；因此新增
宿主页来源不要求修改通用 Token。Desktop Renderer 的 `HostedRouteService` 拥有 namespace 注册、路径解析和
实际导航，Capability Provider 只把已授权命令委托给该服务。其他宿主可以提供不同实现而保持合同不变。

Plugin 与 Theme 分别在自己的 Desktop 集成目录中映射系统权限、创建精确 Grant，并固定 namespace 和
当前 owner。SDK 只看到通用 `HostedRouteRef` 和 Grant/Constraint 合同，不包含 `ui.slot.workspace-view`、
Plugin/Theme namespace 或激活状态。
