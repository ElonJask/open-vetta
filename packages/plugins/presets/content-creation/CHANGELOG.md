...
- 四个领域工具的 description 重写为“正向触发、反向边界、必要前置、副作用、关键返回”的简洁路由合同，并按 `tools/` 下的独立模块拆分名称、模板字符串描述、Schema 与 handler；`content_creation_edit` 的视频方法教程移回按需 Skill/reference，`assets` / `edit` 明确替代路径，关键 action/ID/批次字段和全部 operation 判别分支补齐模型可见语义说明，同时保留 run 全局确认边界。

### Added

- MiniMax H3 本地生成新增 720p、1K 和 2K 三档分辨率，默认使用 1K 档；分辨率选择会按所选宽高比确定性写入 ComfyUI 的宽高控制链路，不再使用模糊质量词、MP 或历史工作流中的尺寸。
...
- Agent 新增高层 `configure_video_shot` 镜头计划：按明确控制需求选择单图、首尾帧或全能参考；首尾帧分别编译静态关键帧提示词，全能参考生成稳定的素材别名、语义角色与媒体引用清单，并通过 inspect 暴露实际策略和方法诊断。
...
- 空画布起步态改为更轻的高级布局：标题浮于选项之上、单层玻璃选择面、线形图标无色块、生成类优先；去掉嵌套卡片与 sparkles 装饰。
- 节点创建入口（空态、节点库、连线创建菜单）统一为中性 muted 图标底，去掉按节点类型分色的 sky/amber/emerald 等彩色底；删除定义中仅用于该配色的 `accent` 字段。
- 移除输入栏「内容创作」开关；活动栏工作区默认上栏，Agent tools/skills 仍在插件启用后直接贡献，不再依赖该 toggle 做软显隐或 prompt 装饰。
