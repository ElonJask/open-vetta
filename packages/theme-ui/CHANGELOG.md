...
- `SidebarPanel` 与 `SidebarDock` 的宽度改为按 `sidebarWidthValue()`（`var(--sidebar-live-width, <width>px)`）取值，并新导出 `SIDEBAR_LIVE_WIDTH_VAR` / `sidebarWidthValue`：宿主在拖拽途中只更新该变量，`width` 作为 committed 值兜底，变量缺失时行为不变。
